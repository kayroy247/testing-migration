...
const ShowYieldFarmDetails = ({ content, wallet, refreshTokenStaked, updateFarmAllowances }) => {
  ...
  const RGPBUSDlpApproval = async (props) => {
    if (wallet.signer !== 'signer') {
      try {
        const poolOne = await smartSwapLPTokenPoolOne();
        ...
        //@ceejay where is the props coming from 
        //I added props as argument to avoid error.
  ...
  const checkUser = async val => {
    ...
    //@ceejay where is checkAllow coming from 
      ...
      <Modal isOpen={modal1Disclosure.isOpen} onClose={close} isCentered="true">
        ...
        <ModalContent bg="#120136" color="#fff" borderRadius="20px" width="90%">
          ...
          <ModalBody className={styles.body}>
            ...
            <Box mt={4}>
              {enoughApproval(content.poolAllowance, content.availableToken) ? <Button
                my="2"
                mx="auto"
                color={
              ...
              >
                {depositValue}
              </Button> : approvalButton(content.deposit)}
              <Button
                my="2"
                mx="auto"
                color="#40BAD5"
